...
$(function() {
	...
	$('.toModForm').on('click', function(){
		...
		$('input[name="pwd"],input[name="email"],input[name="address"]').attr("readonly",false);
		$('select[name="year"],select[name="month"],select[name="day"],select[name="gender"]').prop("disabled",false);
 		$('.originAddr').hide();
		$('.modAddr').show();
		$('.toModForm').hide();
		$('.viewMode').show();
...
/*	// 수정 처리
	...
	$('.backViewMode').on('click', function(){
		$('input[name="pwd"],input[name="email"],input[name="address"]').attr("readonly",true);
